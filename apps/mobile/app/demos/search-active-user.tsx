...
import { searchActiveUser } from '@humanauthn/api-client';
...
export default function SearchActiveUserScreen() {
  const { t } = useTranslation();
  const ns = 'demos.searchActiveUser';
  ...
  const relatedDocs = useMemo(
    () => buildSearchRelatedDocs(t, ns, SEARCH_ACTIVE_RELATED_HREFS),
    [t],
  );
  ...
  const fetchExample = `await searchActiveUser(
  {
    image: "<base64>",
    os: "${biometricOs}",
    ...
    <View style={demoStyles.screen}>
      ...
      <ScrollView contentContainerStyle={demoStyles.scroll}>
        {result == null ? (
          <DemoApiReference
            ns={ns}
            docsUrl={`${DOCS_BASE}/biometrics/search-active-user`}
            docsLabel="docs.verifik.co/biometrics/search-active-user"
            endpoint={t(`${ns}.apiRefEndpoint`)}
            customParamRows={paramRows}
            fetchExample={fetchExample}
            bullets={['bulletApiClient', 'bulletLiveInstead']}
          />
        ) : null}
        ...
        {isLoading ? (
          ...
          <>
            ...
            />
            <DemoMinScoreControl
              label={t(`${ns}.searchMinLabel`, { score: minScore.toFixed(2) })}
              value={minScore}
