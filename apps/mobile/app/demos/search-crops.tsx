...
import { searchCrops } from '@humanauthn/api-client';
...
export default function SearchCropsScreen() {
  const { t } = useTranslation();
  const ns = 'demos.searchCrops';
  ...
  const relatedDocs = useMemo(
    () => buildSearchRelatedDocs(t, ns, SEARCH_CROPS_RELATED_HREFS),
    [t],
  );
  ...
  const fetchExample = `await searchCrops(
  {
    images: ["<base64Crop>", "<base64Crop>"],
    min_score: ${minScore},
    ...
    <View style={demoStyles.screen}>
      ...
      <ScrollView contentContainerStyle={demoStyles.scroll}>
        {result == null ? (
          <DemoApiReference
            ns={ns}
            docsUrl={`${DOCS_BASE}/biometrics/search-crops`}
            docsLabel="docs.verifik.co/biometrics/search-crops"
