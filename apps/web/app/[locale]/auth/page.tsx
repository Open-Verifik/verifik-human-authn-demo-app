...
import { zodResolver as reactHookFormSchemaResolver } from "@hookform/resolvers/zod";
import { z as validationSchema } from "zod";
	...
	const emailSchema = useMemo(
		() =>
			validationSchema.object({
				email: validationSchema.string().email({ message: translateAuth("validationEmailInvalid") }),
			}),
		[translateAuth],
	);
		...
		() =>
			validationSchema.object({
				phone: validationSchema
					.string()
					.min(7, { message: translateAuth("validationPhoneTooShort") })
					.max(13, { message: translateAuth("validationPhoneTooLong") })
					.regex(/^\d+$/, { message: translateAuth("validationPhoneDigitsOnly") }),
			}),
		[translateAuth],
	);

	const emailForm = useForm<EmailForm>({
		resolver: reactHookFormSchemaResolver(emailSchema),
		mode: "onChange",
		defaultValues: { email: "" },
	});
	const phoneForm = useForm<PhoneForm>({
		resolver: reactHookFormSchemaResolver(phoneSchema),
		mode: "onChange",
		defaultValues: { phone: "" },
	});
			...
			<main className="relative z-10 w-full max-w-[400px] bg-white rounded-3xl p-8 shadow-float animate-slide-up text-auth-modal-ink">
				<div className="flex items-center justify-between mb-8">
					<button
						...
						aria-label={translateAuth("backAria")}
						type="button"
					>
				...
				<div className="text-center mb-8">
					<h1 className="text-2xl font-bold tracking-tight text-auth-modal-ink mb-2">{translateAuth("title")}</h1>
					<p className="text-sm text-auth-modal-ink/70">{translateAuth("subtitle")}</p>
				</div>
					...
					<form onSubmit={emailForm.handleSubmit(onEmailSubmit)} className="space-y-4">
						<div>
							<label htmlFor="email-input" className="sr-only">
								{translateAuth("emailLabel")}
							</label>
							<input
								id="email-input"
								type="email"
								autoComplete="email"
								placeholder={translateAuth("emailPlaceholder")}
				...
				{method === "phone" && (
					<form onSubmit={phoneForm.handleSubmit(onPhoneSubmit)} className="space-y-4">
						<div>
							<span className="sr-only">{translateAuth("phoneRowLabel")}</span>
									...
									<Controller
										...
										render={({ field }) => (
											<input
												...
												placeholder={translateAuth("phonePlaceholder")}
				...
				<div className="mt-8 flex items-center justify-center gap-4 opacity-60 text-[9px] font-mono text-auth-modal-ink/55 uppercase tracking-widest">
					<span>{translateAuth("footerCrypto")}</span>
					<span>•</span>
					<span>{translateAuth("footerBrand")}</span>
				</div>
			</main>
		</AuroraBackground>
	);
