...
export default function HumanIDPage() {
	const t = useTranslations("demos.humanid");
		...
		<div className="min-h-screen bg-surface flex flex-col overflow-hidden">
			<ElectronAwareAppHeader className="justify-between">
				...
				>
					...
					<span className="label-meta text-[10px]">{t("cancel")}</span>
				</button>
				<div className="flex items-center gap-2">
					<span className="material-symbols-outlined text-primary" style={{ fontVariationSettings: "'FILL' 1" }}>
						fingerprint
					</span>
					<span className="font-black tracking-wider text-primary text-lg">{t("brandTitle")}</span>
				</div>
			...
			<main className="relative flex flex-col items-center justify-center min-h-screen px-6 z-10 pt-20 pb-12">
				<section className="w-full max-w-md flex flex-col items-center">
					<div className="text-center mb-8">
						<p className="label-meta text-primary text-[0.6875rem] mb-2">{t("kicker")}</p>
					...
					<div className="text-center space-y-4 mb-8 w-full max-w-lg">
						{scanStep === "idle" && (
							<>
								<p className="text-on-surface text-lg font-medium tracking-tight">{t("idleTitle")}</p>
								<p className="text-on-surface-variant text-sm font-light">{t("idleSubtitle")}</p>
							</>
						)}
						{scanStep === "success" && (
							<>
								<p className="text-primary text-lg font-bold">{t("successTitle")}</p>
								<p className="text-on-surface-variant text-sm">{t("successSubtitle")}</p>
							</>
						)}
						{scanStep === "failed" && (
							<>
								<p className="text-error text-lg font-bold">{t("failedTitle")}</p>
								<p className="text-on-surface-variant text-sm">{t("failedSubtitle")}</p>
							</>
						)}
					</div>
				...
				<aside className="absolute bottom-12 right-12 hidden lg:block">
					<div className="text-right space-y-1.5 border-r border-outline-variant/20 pr-4">
						<div className="text-[10px] text-on-surface-variant/55 font-mono">{t("engineLine")}</div>
						<div className="text-[10px] text-on-surface-variant/55 font-mono">{t("captureLine")}</div>
						<div className="text-[10px] text-on-surface-variant/55 font-mono">{t("protocolLine")}</div>
					</div>
				</aside>
			</main>
		</div>
