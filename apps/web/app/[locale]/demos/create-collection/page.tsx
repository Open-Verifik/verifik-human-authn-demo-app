...
export default function CreateCollectionPage() {
	const t = useTranslations("demos.createCollection");
					...
					<form onSubmit={handleSubmit} className="space-y-5">
						<div>
							<label className="block text-sm font-semibold text-on-surface mb-1.5" htmlFor="col-name">
								{t("nameLabel")} <span className="text-error">{tCommon("requiredStar")}</span>
							</label>
							<input
								id="col-name"
								type="text"
								...
								<table className="w-full text-xs border-collapse">
									...
									<tbody className="text-on-surface-variant">
										...
										<tr className="align-top">
											<td className="py-2 font-mono text-primary">{t("paramDescription")}</td>
											<td>{tCommon("no")}</td>
											<td>{t("paramDescriptionDesc")}</td>
										</tr>
									</tbody>
								</table>
							</div>
