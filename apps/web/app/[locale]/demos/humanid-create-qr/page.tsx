...
export default function HumanIdCreateQrPage() {
	const t = useTranslations("demos.humanidCreateQr");
			...
			>
				<details className="mb-8 mt-6 rounded-xl border border-outline-variant/20 bg-surface-container-low/50 text-left px-4 py-3 group">
					...
					<div className="mt-4 space-y-3 text-sm text-on-surface-variant border-t border-outline-variant/15 pt-4">
						...
						<p className="text-xs">
							{t("apiIntroBefore")} <code className="text-primary">encrypt</code>. {t("apiIntroAfter")}
						</p>
						<div className="overflow-x-auto">
							<table className="w-full text-xs border-collapse">
								...
								<tbody className="text-on-surface-variant">
									{ENCRYPT_QR_PARAM_ROWS.map(([param, required, descKey]) => (
				...
				{!hasHydrated ? (
					...
					<HumanIdStructuredResult
						...
						successDescription={t("resultSuccessDescription")}
						onCreateAnother={reset}
						onBackToDemos={() => router.push("/home")}
						createAnotherLabel={tCommon("createAnother")}
					/>
				)}
			</main>
		</div>
