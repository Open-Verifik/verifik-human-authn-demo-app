	...
	return (
		<div className="min-h-screen bg-surface flex flex-col">
			...
			<main className={`flex-1 mt-20 mb-10 px-4 md:px-8 mx-auto w-full ${step === "result" ? "max-w-3xl" : "max-w-2xl"}`}>
					...
					<div className="mt-4 space-y-3 text-sm text-on-surface-variant border-t border-outline-variant/15 pt-4">
						...
						<div className="overflow-x-auto">
							<table className="w-full text-xs border-collapse">
								<thead><tr className="border-b border-outline-variant/20"><th className="text-left py-2 pr-1">Param</th><th className="text-left py-2 pr-1">Req</th><th className="text-left py-2">Description</th></tr></thead>
								<tbody className="text-on-surface-variant">
									{[["publicData","Yes","Object with string values"],["faceBase64","Yes","Base64 face image"],["livenessLevel","Yes","Liveness threshold level"],["metadata","Yes","Object with string values"],["os","Yes","DESKTOP / IOS / ANDROID"],["identifier","Yes","Alphanumeric person identifier"],["requireLiveness","Yes","Require liveness on decrypt"],["tolerance","No","REGULAR / SOFT / HARDENED"],["password","No","Optional encryption password"]].map(([p,r,d])=>(
										<tr key={p} className="border-b border-outline-variant/10"><td className="py-2 font-mono text-primary">{p}</td><td>{r}</td><td>{d}</td></tr>
									))}
								</tbody>
							</table>
						</div>
				...
				<div className="mb-8">
					<h2 className="text-3xl font-black tracking-tight text-on-surface mb-2">{step === "result" ? "HumanID Created" : "Create HumanID"}</h2>
					<p className="text-on-surface-variant">Bind identity data + face into an encrypted ZelfProof on IPFS.</p>
				</div>
				{!hasHydrated ? <div className="h-60 rounded-xl bg-surface-container-high animate-pulse" /> : !canUseDemo ? <DemoSignInPrompt /> : step === "form" ? (
					<form onSubmit={handleSubmit} className="space-y-5">
						...
						<button type="submit" disabled={!faceB64} className="w-full py-3 bg-primary-cta text-on-primary-container font-bold rounded-lg shadow-primary hover:opacity-90 active:scale-95 transition-all disabled:opacity-50">Create HumanID</button>
					</form>
				) : step === "processing" ? (
