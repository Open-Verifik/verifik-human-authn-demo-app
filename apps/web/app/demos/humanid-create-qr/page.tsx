			...
			>
				<details className="mb-8 mt-6 rounded-xl border border-outline-variant/20 bg-surface-container-low/50 text-left px-4 py-3 group">
					...
					<div className="mt-4 space-y-3 text-sm text-on-surface-variant border-t border-outline-variant/15 pt-4">
						<p className="font-mono text-xs text-on-surface">POST /v2/human-id/encrypt-qr-code</p>
						<p className="text-xs">
							Same payload as <code className="text-primary">encrypt</code>. Returns a PNG QR-code data URL (
							<code className="text-primary">data.zelfQR</code>) plus the usual ZelfProof and IPFS metadata.
						</p>
							...
							<table className="w-full text-xs border-collapse">
								...
								<tbody className="text-on-surface-variant">
									{[
										...
										<tr key={p} className="border-b border-outline-variant/10">
											<td className="py-2 font-mono text-primary">{p}</td>
											<td>{r}</td>
											<td>{d}</td>
										</tr>
									))}
				...
				<div className="mb-8">
					<h2 className="text-3xl font-black tracking-tight text-on-surface mb-2">
						{step === "result" ? "HumanID QR Ready" : "Create HumanID QR"}
					</h2>
				...
				{!hasHydrated ? (
					...
					<form onSubmit={handleSubmit} className="space-y-5">
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
							<div>
								<label className="block text-sm font-semibold text-on-surface mb-1.5" htmlFor="qr-identifier">
							...
							<div>
								...
								<select
									value={tolerance}
									onChange={(e) => setTolerance(e.target.value as "REGULAR" | "SOFT" | "HARDENED")}
						...
						<div className="flex items-center gap-3">
							...
							/>
							<label htmlFor="qr-req-live" className="text-sm text-on-surface">
								Require liveness on decrypt
							</label>
						</div>
					...
					<HumanIdStructuredResult
						result={result}
						zelfQrPngDataUrl={qrDataUrl}
						successTitle="HumanID QR ready"
						successDescription="PNG QR, ZelfProof, IPFS details, and usage below."
