...
export default function SearchPersonResult({ result, onReset }: SearchPersonResultProps) {
	const t = useTranslations("demos.searchPersonResult");
	...
	const parsed = useMemo(() => {
		const rawMatches = Array.isArray(result?.data) ? result.data : [];
		...
		return { matches, signature, requestId };
	}, [result, tCommon]);
			...
			{activeTab === "matches" ? (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<p className="font-bold text-on-surface">{t("searchResults")}</p>
