			...
			{activeTab === "matches" ? (
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<p className="font-bold text-on-surface">Active user search results</p>
					...
					{parsed.liveness ? (
						<div className="rounded-xl border border-outline-variant/20 bg-surface-container-high/40 p-4">
							<p className="font-semibold text-on-surface mb-2">Liveness</p>
							<dl className="space-y-1.5 text-sm">
								...
								{parsed.liveness.passed !== null ? (
									<div className="grid grid-cols-1 sm:grid-cols-[minmax(0,10rem)_1fr] gap-x-3 gap-y-1">
										<dt className="text-on-surface-variant font-medium">Passed</dt>
										<dd className={parsed.liveness.passed ? "text-emerald-400" : "text-error"}>
											{parsed.liveness.passed ? "Yes" : "No"}
										</dd>
									</div>
								) : null}
								{parsed.liveness.score !== null ? (
									<div className="grid grid-cols-1 sm:grid-cols-[minmax(0,10rem)_1fr] gap-x-3 gap-y-1">
										<dt className="text-on-surface-variant font-medium">Score</dt>
										<dd className="text-on-surface">{formatScore(parsed.liveness.score)}</dd>
									</div>
								) : null}
							</dl>
						</div>
