...
const { getWebDevPort } = require(path.join(__dirname, '../web/scripts/dev-port.cjs'));

const devWebOrigin = () => `http://127.0.0.1:${getWebDevPort()}`;
const devWebOriginsCsp = () => {
    const p = getWebDevPort();
...
const loadInMainWindow = (routePath) => {
    const nextUrl = isDev ? `${devWebOrigin()}${routePath}` : getProductionUrl(routePath);
    return mainWindow?.webContents.loadURL(nextUrl);
};

/**
 * HumanAuthn web uses next-intl with defaultLocale `en` and localePrefix `as-needed`.
 * English URLs stay unprefixed (`/home`, `/demos/...`). Spanish and other locales use
 * a prefix (e.g. `/es/home`). These menu paths intentionally target the default English
 * routes so no Electron changes are required for the primary locale.
 */
const DEMO_ROUTES = [
...
function getProductionUrl(routePath) {
...
function createWindow() {
    ...
    // ── Load URL ─────────────────────────────────────────────────────────────────
    const startUrl = devWebOrigin(); // production: could use file:// with custom server
        ...
        callback({
            responseHeaders: {
                ...
                'Content-Security-Policy': [
                    `default-src 'self' ${devWebOriginsCsp()} https://verifik.app https://api.verifik.co https://*.verifik.co https://fonts.googleapis.com https://fonts.gstatic.com; ` +
                    `script-src 'self' 'unsafe-inline' 'unsafe-eval' ${devWebOriginsCsp()}; ` +
                    "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; " +
                    "img-src 'self' data: blob: https: http:; " +
                    "media-src 'self' blob:; " +
                    `connect-src 'self' ${devWebOriginsCsp()} https://verifik.app https://api.verifik.co https://*.verifik.co https://access.verifik.co ws://localhost:*;`,
                ],
            },
        });
    });
